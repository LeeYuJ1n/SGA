...
{
	// 아래에 주석 처리된 부분은 방법 A, B, C를 위한 코드
	// trees.reserve(COUNT);
	// 
	// FOR(COUNT)
	// {
	...
	// 	transform->Pos() = tree->Pos(); // 위치 넣기
	// 	origins.push_back(transform); // 벡터의 추가
	// }
	// 
	// // 캔버싱용 변수들 준비
...
void BillboardScene::Update()
{
	...
	// 	tree->Rot().y = atan2(dir.x, dir.z); // 나무의 방향을 (역) 탄젠트로 주기
	...
	// 	// find : 벡터에 사용하면 반환값은 반복자(iterator). 이 자체로는 숫자 계산 불가능
...
void BillboardScene::Render()
{
	...
	// 출력장치에서 직접 출력 호출
	DC->Draw(COUNT, 0); // 미리 설절된 이미지 정보를, COUNT만큼, 0번 정점부터 돌아가면서 출력

	// 출력장치에 셰이더 설정 (출력 과정에서 셰이더가 연산을 통해 매리티얼을 직접 그려낸다)
	DC->GSSetShader(nullptr, nullptr, 0); // 매개변수 의미 : 1 : 수동 연결할 픽셀 셰이더 없음
	                                      //                 2 : 추가 설정할 클래스 인스턴스 없음
	                                      //                 3 : 클래스 인스턴스 배정 개수
	                                      //                 = "사전에 설정해둔 그대로"

	// 출력장치를 통한 직접 출력 ( = 렌더) 종료 후, 원상복수
...
void BillboardScene::PostRender()
{
	...
	// 스크린에서 텍스처를 투사한다면 여기서 호출
	// blendState[1]->SetState(); // 텍스처용 블렌드 상태 적용
	// 
	// for (Quad* tree : trees)
	// 	tree->Render(); // 나무 출력
	// 
	// blendState[0]->SetState(); // 원상 복수
}
