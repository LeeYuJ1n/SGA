...
private:
    ...
    //     -> DX의 기능 사용 (지오메트리 셰이더) : 텍스처이 정보를 화면에 직접
    //        (그래픽 연산부터 하고 -> 2D의 정보를 가진 데이터로 렌더)
    //        하드웨어 부담이 있지만, 연산과 셰이더가 지원되는 환경에서는 속도가 더 빠르다
    //        랜덤 생성의 경우 어떻게 데이터를 보존할(혹은 필요에 따라 셔플할) 것인가가 관건
    //        렌더 호출 위치가 코드 안에서 비슷하기 때문에 상호작용, 출력 우선순위가 비교적 안정적

    // 빌보드 사용 사례(용도) : 먼 곳의 배경, 플레이어가 놓치면 안 되는 2D 정보
    //                          (2D 텍스처로 나온 줍는 아이템, 홀로그램 정보창, 온라인 게임의 플레이어 이름 등))
    //                          게임 중에 나오는 그래픽 특수 효과들(GFX, 혹은 파티클)
