...
public:
	...
	int FineCloseNode(Vector2 pos); // "지정받은 지점에서 가까이 있는 노드들 검색" : 주변에 뭐가 있나?
	int FindMinCostNode();          // 최소비용 경로 검색

	void SetEdgeInfo(int index); // 경로정보 세팅
	void ResetNodes();           // 노드 (수행) 리셋

	vector<Vector2> GetPath(int start, int end); // 경로(경우의 수) 보존용 (장기보존 필요가 없으므로 함수로만)

	// 씬에서 노드 추가각 필요한 경우, 추가 함수 작성 후 CreateNode함수를 토스하듯이 추가
