...
DNode::DNode(Vector2 pos)
{
	...
	// 콜라이더 생성 ( * 알고리즘보다는 길 찾기 이후 / 이전에 좀 더 필요한 부분)
	collider = new CircleCollider(30); // 반지름 30짜리 원 그리기
	collider->Pos() = pos;
	collider->UpdateWorld(); // 만들고, 콜라이더도 만들고, 1회 업데이트 (생성 위치 반영만 해주기)
}
...
void DNode::AddEdge(DNode* node)
{
	...
	// D 알고리즘에서 경로가 있으면 그 경로에는 비용이 필수로 들어가야 한다
	// 단, 여기서는 해당 비용을 "두 노드 사이의 거리"로 가정한다
	// (다른 비용 측정 방법이 있다면, 해당 연산으로 교체)

	// 타 노드와 이어지는 경로 설정, 비용 측정 끝나면 벡터에 넣기
