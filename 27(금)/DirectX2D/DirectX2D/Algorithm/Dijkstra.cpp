	...
	if (KEY_UP(VK_RBUTTON))
	{
		...
		for (DNode* node : nodes)
		{
			...
			if (node->collider->IsPointCollision(mousePos)) // 그리고 이 노드가 커서 위치와 겹치면
			{
				...
				// selectNode = nullptr;   // 옵션 : 선택을 완전히 해제하고 싶은 사람만
				break;
			}
		}
	}

	// 길을 찾아가는 기능도 필요한데, 길 찾기 클래스 내에서는 "길을 누가 찾아갈 것인가?"가 빠져 있으므로 보류
...
int Dijkstra::FineCloseNode(Vector2 pos)
{
	...
	int result = -1; // 결과 기본값은 "업음"의 의미를 가지는 -1
	...
	for (int i = 0; i < nodes.size(); ++i)
	{
		...
		{
			minDist = distance; // 기준를 갱신해주고
			result = i; // 결과 반영
		}

		// 이 반복문이 끝나면 지정된 위치에서 가장 가까운 "주변" 노드 검색이 완료
	}
...
void Dijkstra::SetEdgeInfo(int index)
{
	...
	for (DNode::Edge* edge : nodes[index]->edges)
	{
		if (edge->node->cost > cost + edge->cost)
		{
			...
			// * 노드의 비용 기본값은 최대지만, 이 함수가 호출될 떄, 시작지점 비용은 0이 되었기 때문에 0부터 비용을 점증시킬 수 있다

			edge->node->via = index; // 노드의 경유지를 매개변수로 받은 인덱스로 등록 (이 경유지가 경로 역추적 때 쓰인다)
		}
	}

	nodes[index]->isColsed = true; // 모든 계산이 끝난 후, ( = "나한테서 나한테로"가는 경우 빼기)
}
...
{
	if (start == -1 || end == -1) return vector <Vector2>(); // 시작이나 끝이 없을 경우 ( = 경로 성립이 안 되면 텅 빈 벡터주기) -> 그냥 종료
	...
	// 이 시점에서도 이론적으론 경로 설정 시도할 수 있지만, 더 확실하게 하려면 이후의 연산
	...
	while (curIndex != start) // 선택 인덱스가 시작이 될 때까지
	{
		...
		curIndex = nodes[curIndex]->via; // <- 정보 추출 과정 어딘가에서 via가 각 노드에 재설정이어야 한다
		// 여기로 오면 노드에서 뒤에서부터 각 노드의 위치와 경유지가 차례로 벡타와 curIndex에 등록
	...
	//      (거리가 긴 직성 1개와 짧은 직선 여러 개를 비교한다는다...)
