...
class DNode
{
	friend class Dijkstra; // 싱글턴처럼 순서 상관없이 데익스트라 클래스와 연관성을 주고 받으므로 미리 선언

	struct Edge // Edge : D 알고리즘에서 각 D 노드와 연관된 노드의 여부, 그 사이에서 생긴 연관성 정보를 뜻한다
		        // (D 알고리즘에서 이동은 결국 점에서 점이 아니라 "지역에서 지역"이라는 의미를 포함하고 있다는 의미도 있다)
	{
	...
	// D - 노드 : 데익스트라 알고리즘애서 위치 정보를 담당하는 클래스
	//            (위치 : 시작점, 목적지, 경유지 중 하나)
...
private:
	...
	bool isColsed = false; // 이 노드가 외부에서 접근 가능한가 ("닫혔는가?") 여부

	int index = 0;  // 노드들 사이에서 "이" 노드가 가질 인덱스
	int via   = -1; // 경유 여부, 혹은 경유지 정보, 혹은 경유 비용 등 경유 상황에서 쓰일 변수
	                // "vla" : ~~를 통해서 (Ban via PickC : 캐릭터 선택 중의 배제 = 픽밴, 사람 밴이 아님)
	...
	Collider* collider; // * 길 찾기와 별도로 길 찾기 후 이동 연산에 (주로) 사용할 콜라이더
};
