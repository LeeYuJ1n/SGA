...
public:
	...
	// dataSize    : 데이터가 들어갈 자리 미리 지정

	// Reader 전용 함수 : "파일에서 데이터가 없었습니다"를 반환받기 위한 함수
	bool IsFailed() { return (file == INVALID_HANDLE_VALUE); } // 핸들링 결과가 없거나, 있어도 부적절하면 "실패" 함수가 true
	bool IsValid() { return (file != INVALID_HANDLE_VALUE); } // 위 함수와 똑같은
