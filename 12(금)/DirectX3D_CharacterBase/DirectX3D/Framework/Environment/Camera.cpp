...
void Camera::Frustum()
{
    //프러스텀(절두체) : 뿔 모양의 3차원 도형에서 뿔의 꼭대기를 아래 면에 평행하게 자른 것
    //                 컴퓨터에서는 특별히 모니터의 모양에 맞추어 평행면이 사각형인 것을 지칭한다

    //절두체의 목적 : 카메라의 시야에 물체가 보이는지 여부를 판별하기 위함

    //절두체의 활용 : 1. 시야에 없는 물체를 그리지 않기 (프러스텀 컬링 : 렌더 연산 아끼기)
    //               2. 시야에 물체가 들어와 있는 상황을 정보로 활용하기 (이벤트 진행)
    //               3. 시야에 들어온 물체에 대해 상호작용하기 (조준 보정 혹은 "저승사자의 시선" 스킬 등)

    Float4x4 VP; //뷰포트 임시 변수
    XMStoreFloat4x4(&VP, view * projection); //뷰포트의 모양을 낸다
                                             //모양 내는 방법 : *뷰포트 행렬(2차원 같은 3차원) x 프로젝션 행렬(실제 3차원)*
                                             //                이 관계의 이해가 중요! (행렬의 곱은 어떤 방식으로든 하니까)
                                             // 
                                             // -> 행렬곱 결과 (4x4행렬 x 4x4행렬 = 4x4 행렬)
                                             //               행렬 모양은 같은데, 면의 모양이 달라진다
                                             //               -> 원래의 뷰포트 모양이 프로젝션 행렬의 영향을 받아 팽창하게 된다
                                             //               -> 그 결과가 바로 절두체


    // 만든 새로운 뷰포트 (3차원 모양, 원근 시점, 앞은 좁고 뒤로 갈수록 팽창하는 모양)
    ...
    planes[0] = XMVectorSet(a, b, c, d); // DX 제공 함수로 벡터 공간 세팅 (2차원 평면)
                                         // 행렬곱이 아니라 실제 적용될 "면"을 구하는 게 목적이라 DX 함수가 유리하다 (필수까진 아님)
    ...
    //->여기에서 어떤 거리를 주게 되면? -> 그게 이 절두체의 전후 한계(범위)
...
bool Camera::ContainPoint(Vector3 point)
{
    ...
    // 1. 씬. 절두체 안에 씬의 개체가 들어오면 렌더, 아니면 렌더 없음

    // 2. 게임매니저. 절두체 안에 게임의 모든 개체가 들어오면 렌더, 아니면 없음.
    ...
    // -> 다만 최적화나, 개발상의 편의를 위해서 일반적으론 1~2 정도의 위치에 절두체 관련 호출을 한다
