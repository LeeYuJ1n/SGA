...
Naruto::Naruto() : ModelAnimator("Naruto")
{
	...
	// 앱 시작시 마우스 위치에 의해 시점이 멋대로 돌아가는 걸 줄이기 위해
...
void Naruto::Move()
{
	...
	if (KEY_PRESS('W'))
	{
		...
		isMoveZ = true; // 전후 이동 주행 중
	}

	// TODO : 후진, 좌우 이동도 같은 방식으로 작성
	...
	if (!isMoveZ) // 전부 이동 중이 아니면
	{
		velocity.z = Lerp(velocity.z, 0, deceleration * DELTA); // 보간에 의해 감소
	}

	// TODO : x 축에도 같은 계산이 필요합니다!

	// 좌우회전가 행렬계산에 의한 면법선 내기 ( = 정면 구하기)
	Matrix rotY = XMMatrixRotationY(Rot().y);
	Vector3 direction = XMVector3TransformCoord(velocity, rotY);  // 현재의 공간이 가지는 "정면" 실제 벡터 방향

	Pos() += direction * -1 * moveSpeed * DELTA; // "정면 방향대로 이동"
	// * -1 : 캐릭터가 뒤집어져 있어서 (3D fbx나 nif모델을 직접 사용할 경우 종종 일어나는 일)
...
void Naruto::SetAnimation()
{
	...
	// * 위 조건문에서 0.1이 더 작은 수로 바뀔수록 동작이 자연스럽고, 동작 전환은 민감하고
	//   경우에 따라 부자연스러울 것 (그 사이에서 자기 균형을 찾아야 함)
}
