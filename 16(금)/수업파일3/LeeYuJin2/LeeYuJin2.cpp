...
int main()
{
    ...
    // 예시 : "헬로 월드" 10번 연속 출력하기

    // 만약에 이 코드를 직접 입력을 해야 한다면...
    // std::cout << "헬로 월드!" << std::endl; 이 코드를 100번 반복!

    // 반복문 사용하는 방법 : 가장 대표적인 명령어
    // for (시작 상황; 반복문이 계속 될 조건; 지정된 코드가 끝날 때마다 변화가 일어날 경우 그 변화)

    // for 직접 써보기(헬로월드 100번)

    for (int i = 0; i < 100; ++i)
        // int i = 0 : 이 반복문을 시작할 때, "i"라는 이름으로 정수를 만들고 0으로 할당한다(시작 상황)
        // 여기서 i는 for 문에서 보편적으로 선택되는 정수 변수의 이름(차례, 즉 index, 혹은 iterator의 첫 글자)
        // i가 왜 0인가 : 컴퓨터에서는 0이 첫 번째 숫자, 혹은 기준으로 자주선택되므로 거기 맞춘 것
    ...
    // for문의 다른 활용법 : 규칙적인 변화와 "반복 횟수"를 활용하는 방법
 
    // for의 두 가지 특징 : 1. 코드가 끝나면 규칙적으로 특정 조건이 바뀐다
    // 2. 해당 조건의 for문의 시작과 끝이 달려있기 때문에, 결과적으로 for문은 "반복 홋수가 정해져 있다!"

    // for 문의 활용 사례 : 구구단(1부터 9까지 차례로 특정 숫자를 곱하기)
    ...
    for (int i = 0; i < 10; ++i)
    {
       // i가 0부터 9일때까지 10번 반복하는 코드
       
       // 반복문 안에 반복문

        for (int j = 0; j <= i; ++j)
            // 반복문 안에 반복문을 적을 때, 보통은 i 다음 j 다움 k 등으로 차례를 담당한다
            // i가 0일때는 j도 0일 때만, i가 9라면 j도 0부터 9가 될 때까지 다른 횟수로 반복
        {
            std::cout << "*";
        }

        std::cout << std::endl; // i가 코드가 끝날 때만 줄바꿈을 하겠다
    }
}