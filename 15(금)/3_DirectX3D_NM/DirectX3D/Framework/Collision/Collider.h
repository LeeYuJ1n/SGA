#pragma once
#include "Objects/Basic/GameObject.h" // 충돌체 헤더를 먼저 읽었을 경우 게임 오브젝트 내용을 참조하기 위함(상호참조)
...
struct Ray
{
	...
	Ray() {}
	Ray(Vector3 pos, Vector3 dir) : pos(pos), dir(dir) // 유니티 스타일
	{
		// this->pos = pos;
		// this->dir = dir;
	}
...
{
	string tag; // 접점의 이름
	// ↑ 충돌 판정을 비록한 레이캐스팅 결과 반환이 일어났을 때, 해당 경우가 필요한 상황인지 알려면
	// 1. 충돌에 개입한 물건의 이름
	// 2. 광선의 현재 정보
	// 그런데 1, 2만으로 충분하기 않은 경우 (물건에 이름이 없고, 광선의 정보 대상이 물체 간 상호작용일 때)
	...
	Vector3 hit;
	float distance; // 거리는 "그냥 숫자"라서 그냥 숫자( = 스칼라)
};

// 광선이 접점을 활용하는 단순한 형태의 충돌체 클래스 만들기
// + 인터페이스로 만들어서 호환성 높이기

// class Collider : public Transform // 2D에서의 방식, 기본형에서 속성을 이어받는 구조
class Collider : public GameObject   // 기본형(트랜스폼과 기본 게임 개체)이 있고, 거기에 새로운 속성을 부여하는 컴포넌트 구조
{
protected: // 상속받은 실제 충돌체와 공유할 수 있도록
	enum class Type
	{
		// RECT, CIRCLE    // 직사각형, 원 : 2D 명칭
...
public:
	...
	// 매개변수에 쓰인 IN과 OUT : 실제 코드는 아니고, 그냥 나주엥 봤을 때 각 매개변수의 역할을 봤을 때 각 매개변수의 역할을 잘 보기 위한 메모
	// IN Ray ray           : 함수에 광선 정보"를" 쓰겠다
	// OUT Contact* contact : 이 함수의 결과는 별도의 반환형이 아니라 이 구조체는 담겠다

	virtual bool InSphereCollision(class SphereCollider* collider) = 0;
...
protected:
	virtual void MakeMesh() = 0; // 트랜스폼의 메시 만들기 : 이번에는 충돌체가 직접 만들어야 하니까

	Type type; // 충돌체 실제 유형

	Mesh<Vertex>* mesh; // 정점으로 만든 모양 틀

	bool isRender; // 렌더 여부(트랜스폼을 안 받는대신 직접 만들기) : 렌더를 안 해도 충돌은 해야 할 수 있으니까
