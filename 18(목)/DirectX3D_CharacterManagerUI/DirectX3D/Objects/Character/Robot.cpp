...
void Robot::UpdateUI()
{
    barPos = transform->Pos() + Vector3(0, 180, 0); // Y 180 : 엑터 크기 (모델이 바뀌면 이쪽도 수정)

    hpBar->Pos() = CAM->WorldToScreen(barPos); // 월드상이 개체를 2D 화면으로 투사
                                               // UI의 캔버싱

    float scale = 100.0f / velocity.Length(); // 표적과의 거리에 따라 그림 크기가 달라지게
