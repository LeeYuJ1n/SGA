	...
	, index(index)           // 인스턴싱 내 순번
{
	...
	// 모델의 의해서 root가 바뀌기 전에 충돌체 생성
	collider = new CapsuleCollider(30, 100);   // 반지름 : 30, 높이 : 100
	...
	// 이벤트 설정 (현재 만들어진 것만)
	SetEvent(STAND_UP, bind(&Robot::EndStandUp, this), 0.7f); //로봇에 있는 EndStandUp를 this(여기에) 바인드 한다
	SetEvent(HIT, bind(&Robot::EndHit, this), 0.9f);
	SetEvent(DYING, bind(&Robot::EndDying, this), 1.0f);
...
void Robot::Update()
{
	...
	// 업데이트에서 속력 벡터를 계산해야 한다면 이쪽 코드를 활성화
...
void Robot::Hit(float amount)
{
	...
	// TODO : 체력바 등이 있다면 여기서 추가 UI 업데이트 추가
...
void Robot::Spawn(Vector3 pos)
{
	...
	transform->Pos() = pos; // 매개변수로 받은 위치로
}
...
void Robot::EndStandUp()
{
	...
	collider->SetActive(true); // 콜라이더 활성화가 안 되었다면 활성화시키기
	SetState(WALK);
}
...
{
	// 죽었으니까 없어지면 된다
	// 만약에 시간을 지연시키고 싶으면 : 충돌체만 없앤 다음 약간 시간은 더 주고 다른 함수로 트랜스폼까지 없애주면 된다

	transform->SetActive(false);
	collider->SetActive(false); // 콜라이더 비활성화
}
...
void Robot::Move()
{
	...
	if (velocity.Length() < 10) return; // 거리에 영향을 줘야 할 중간변수(여기서는 "대상과의 거리")가 너무 적어도 종료
	...
	velocity = target->GlobalPos() - transform->GlobalPos(); // 표적과 자신이 거리관계에 따라 속력 벡터
	// GlobalPos() : 현재 위치

	// 표적이 가까이 가기 전에는 천천히 다가가서, 거리가 좁혀지면 달려들기
	if (velocity.Length() < 1000) // 속력 벡터의 길이 ( = 표적과의 실제 거리)가 지정된 수 미만이면
	{
		moveSpeed = 100.0f; // 빠른 이동 시작
		SetState(RUN); // 동작 : 달리기
	}
	else // 거리가 너무 멀면
	{
