...
void Model::SetShader(wstring file)
{
	for (Material* material : materials)
	{
		material->SetShader(file); // 그림 하나한에 셰이더 다 적용
	}
}
...
void Model::ReadMesh()
{
	...
	{
		// return;        // 한다면 즉시 종료 (기본)
		assert(false);    // "무결성 강제 함수" 실행하면서 false 값을 고의로 주기 : 즉시 종류되면서 오류 메시지까지 만들기
	...
	FOR(size)
	{
		...
		// 받아온 정점, 인덱스, 각 정보의 총 개수를 주고, 피드백하면서 멧 생성
	...
	for (BoneData& bone : bones)
	{
		...
		boneMap[bone.name] = bone.index; // 맵에 새 뼈 추가
	}

	// 여기까지 오면 형태, 관절 뼈대 정보를 모두 찾아냈다
	// -> 끝

	delete reader;// 만든 인스턴스는 함수가 끝날 때 바로 폐기
}
