...
void TerrainPicker::Update()
{
    ...
    {
        pickingPos = Pick(); // 찍고 있는 좌표는 찍기 함수의 결과다
    }
}
...
Vector3 TerrainPicker::Pick()
{
    ...
    // -> 마우스 커서에서 시작된 광선(레이캐스트)이 물체에 부딪쳐서 나온 내부의 좌표를 컴퓨터에게 반환

    //레이캐스트 필요
    Ray ray = CAM->ScreenPointToRay(mousePos);

    // 만들어진 터레인의 가로 세로를 모두 검사해서 충돌이 일어났는지 검사.....해야 되는데
    ...
    for (UINT z = 0; z < height - 1; ++z)
    {
        for (UINT x = 0; x < width - 1; ++x)
        {
            ...
            //정점과 인덱스(가로세로 좌표)를 대조해서 실제 위치 받기
            Vector3 p[4];
            for (UINT i = 0; i < 4; ++i)
            {
            ...
            //TriangleTests: DX에서 주는 레이캐스트 및 폴리곤(3차원 표현단위) 간의 충돌을 시험하기 위한 디버그 기능 집합
            //               (DX는 레이캐스트도 지원하고 폴리곤도 지원하지만, 이들 간의 상호작용 지원은 기본적으로는 하지 않는다....제한적으로 이렇게 활용은 가능
            if (TriangleTests::Intersects(ray.pos, ray.dir, p[0], p[1], p[2], distance))
            {
                //폴리곤 및 선분(레이캐스트) 접점이 생겼으면 해당 정보를 그대로 계산해서 벡터로 반환
                return ray.pos + ray.dir * distance; // 광원에서 접점까지 거리만큼, 광선 방향으로 가면 해당 벡터 나옴
                //return height - (ray.pos + ray.dir * distance); // 그림이 뒤집혔을 경우
            }
    ...
    return Vector3(0, 0, 0); // 0, 0, 0 기본 결과 반환
}
...
void TerrainPicker::MakeNormal()
{
        ...
        // 정점 3개가 모일 때 조각으로 된 면 하나가 나오니까
    {
        ...
        //새로 만들어진 법선의 정보를 지금 쓰이는 정점에 포함된 법선정보에 기입
        ...
        //원 정보에 계속 법선정보를 누적시켜서, 해당 정점에 대한 곡면 정보를 완성
...
void TerrainPicker::MakeTangent()
{
    ...
    for (UINT i = 0; i < indices.size() / 3; ++i)
    {
        ...
        // 그림이 기준이 되려고 하니까 그림=UV 좌표도 가져오기
