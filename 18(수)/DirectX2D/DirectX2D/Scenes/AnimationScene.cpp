...
// 새로운 셰이더 파일과 설정을 쓸 것이기 때문에
// 18일 내용에서 본 씬은 아마 의도한 만큼의 렌더가 되지 않을 겁니다
...
{
    // 글꼴 렌더 준비
    // Device::Get()->Clear();         // DC를 준비해야 맞겠지만, 게임 매니저에서 이미 이 코드가 쓰여서 생략
    ...
    // 글꼴 출력
    wstring fpsText = L"FPS : " + to_wstring(Timer::Get()->GetFPS());

    Font::Get()->RenderText(fpsText, { 100, WIN_HEIGHT - 100 });
