...
void BlockManager::CreateBlock(UINT width, UINT height)
{
	...
	{
		textures[i] = L"Textures/Block" + to_wstring(i) + L".png";
	}
...
bool BlockManager::buildBlock(Cube* block)
{
	...
	block->Pos() = collidBlock->GlobalPos() + dir; // 내가 상호작용한 "위치" 블록에서 위에서 만든 dir 만큼 비껴나간 위치
	block->Update();
...
Cube* BlockManager::GetCollisionBlock()
{
	...
	list<Cube*>::iterator it = blocks.begin();        // 돌리면서 검사할 순서
	...
	{
		// 여기까지 왔으면 colliBlock은 계속 리스트의 끝 = 한계 끝 = 충돌한 큐브 없었음
		return nullptr;
	}
}
