...
// 전역 구초제 : 3D 모델을 주출한다면 어디서든 여기서부터 호출할 수 있도록
// -> 이렇게 만들었다가 나중에 번잡하면 네임스페이스를 따로 만들면 된다
...
struct MeshData
{
	...
	UINT materialIndex; // 사용된 메티리얼 데이터
...
// 중심형상 or 프레임의 키 = 애니메이션의 있는 경우 몇 번째 프레임인가?
//                          혹은, 해당 프레임에서 이 모델은 지금 어떤 모양인가?
struct KeyFrame
{
	string bonename;                 // 이 프레임을 적용할 뼈대
	vector<KeyTransform> transforms; // 프레임이 적용된 트랜스폼들
};

// 클림의 구성요소 : 프레임을 모으거나 나누어서 만들어진 "동작"을 담기 위한 구조체
struct ClipNode
{
	aiString name;                   // 에셋 임포트 라이브러리에서 사용된 자료형을 적용
	                                 // (불러올 때 에셋 임포트를 쓸 테니까)
...
struct Clip
{
	...
	float tickperSecond;  // 프레임 재생 속도
	                      // (tick : 프레임의 다른 이름, 동작 재생 중 한 프레임, 혹은 한 순간을 구분해서 지칭하는 용어)
