...
#include <math.h> // 매스 헤더 : 각종 수학 관련 정해진 알고리즘을 불러오기 위한 헤더
...
void AngleScene::Render()
{
    ...
    // 어제 코드의 복습을 겸해서, 색깔(펜을 이용) 생성 후, 직선을 그리고, 펜 삭제까지

    HPEN pen = CreatePen(PS_SOLID, 2, RGB(255, 0, 0)); //펜 생성
    HGDIOBJ oldPen = SelectObject(hdc, pen);           //윈도우가 펜을 선택한 후(selectobject) 남은 기존 펜을 oldPen에 백업

    //직선 그리기
    // 1. (컴퓨터 속 가상 입력 도구의 좌료를) 지정 좌표로 움직이기
    MoveToEx(hdc, 300, 300, NULL); //매개변수 뜻 : 이 모니터에, x 100, y 200, 과거 좌표 참조 안 함
                                   //             여기 위치로 보이지 않는 기준점을 설정

    // 2. (가상 입력 도구의 좌표를) 이동시키면서 직선 궤적을 그림으로 남김
    LineTo(hdc, 300, 200); //매개변수 뜻 : 이 모니터에 x 100, y 400 위치로 입력도구를 이동하면서 그 궤적을 직선으로 그린다
    ...
    // .....그러면, "삼각형을 그리고 싶을 때는?"
    // 그리고 그냥 삼각형이 아니라, 각도가 정해진 삼각형을 그리고 싶다면???
    ...
    // *삼각함수를 쓰려면 해당 기능을 포함 (맨 위에 include)
    ...
    //300 : 위 MoveToEx에서 지정된 시작위치. 즉 원점.

    // 삼각함수 코드 자체는 위에 있는 만큼만 알고 숙지해도 코딩에 충분!
    ...
    //       따라서 con, sin 함수에 1을 넣으면 "호도 1"을 계산한다
    ...
    //          지정한 숫자에 원주율을 곱하고 180으로 나누면 된다

    //          ....보통은 cpp 위쯤이나 framework.h에 미리 define을 해두는 경우가 많다 ㅎㅎㅎ
    //        (예 : #define PHI 3.141592) 나중에 파일 모음 등에서 확인할 기회가 있을지도

    // 2. 윈도우는 y 값이 커질수록 출력 위치가 (화면 좌표가) 밑으로 내려온다 (수학 좌표와 반대)
    ...
    x = x + (cos(GetRadian(60)) * 100); //< -여기서 각도가 60도로 딱 나와야 하는데!
    //각도를 어떻게 구할까? : 호도나 각도를 따지기 전에, 그냥 각도를 함수로 직접 구하면 어떨까?
    // 그래서 헤더에서 함수 작성 후 여기서 호출 (+원주율도 미리 매크로로 만들자)
    y = y + (-sin(GetRadian(60)) * 100);

    LineTo(hdc, x, y); // 정삼각형의 위쪽 꼭지점 (60도 위로 간 곳)

    x = x + (cos(GetRadian(-60)) * 100);
    y = y + (-sin(GetRadian(-60)) * 100);

    LineTo(hdc, x, y); // 정삼각형의 아래 오른쪽 꼭지점 (60도 밑으로 간 곳)
...
float AngleScene::GetRadian(float _degree)
{
    //각도를 호도로 나누기
    return (3.14f / 180) * _degree;
}
