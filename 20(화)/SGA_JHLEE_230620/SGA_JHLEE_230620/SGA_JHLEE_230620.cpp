...
int main()
{
    ...
    // 
    //결과변수 : 해당 조건문의 최종 결과가 저장될 변수
    // (bool이 자주 선택되지만 굳이 bool일 필요는 없다)
    //조건1 : 최종 결과의 참/거짓 여부를 가리기 위한 조건
    // ? (조건1 뒤의 기호) : 이 조건이 맞는지 아닌지를 가리라는 연산자(=삼항연산자)
    //결과1 : 조건1이 참일 때 적용되어야 할 결과
    // 결과1 다음에 오는 : 기호 : 결과1과 결과2를 나누는 표시
    //결과2 : 조건1이 거짓일 때 적용되어야 할 결과
    
    // 조건의 수가 하나밖에 지정될 수 없지만, 결과의 형태가 참/거짓으로 깔끔한 경우
    // if에 비하면 연산이 월등히 빠른 코드
    ...
    switch(CNO)
    {
    case 0:     //CNO가 0인 경우에 아래 코드를 수행하라는 뜻
    ...
    case 3:
        ...
        //switch문이라도 case 하나하나마다 대응되지 않는 예외는 존재
    ...
    // -> switch도 조건문 : if 안에 switch 가능, switch 안에 if 가능
    // switch 안에서 또 switch 가능
    ...
    // - CNO의 사례를 9까지 만들어보기
    // 혹은
    // - game이라는 int를 만들어서, 0부터 9까지 case를 구현하고
    // --각각 cout으로 자기가 좋아하는 게임 제목을 출력을 해봅시다

    // switch의 특징
    // - 조건을 하나밖에 안 본다 (숫자든, bool이든...)
    // - 조건의 형태를 무조건 명확하게 구분할 수 있어야 한다
    //   (다른 표현으로, "int"로 표현할 수 있는 자료형만 조건으로 쓸 수 있다)
    //   (또 다른 표현으로, float 사용 불가.
    //   char는 문자 코드에 대응하는 일종의 암호/변환 표현이라서 사용 가능
    //   (=문자는 숫자로 C++에서 변환 가능!))
    ...
    // - 반대로 case 1일 때도, case 2일 때도 똑같이 실행되어야 하는 코드가 있다면, 고의로 break를 빼고
    // - 복수 조건에 대한 공통 결과를 작성해줄 수도 있다
    ...
    // -> 이런 경우라면 if문이 가장 좋다 (조건문에서 가장 많은 경우)

    // 조건 자체는 하나로 한정된 경우
    // 이 조건문에서 경우의 수가 정확하게 나와 있는 경우 (case1, case2....)
    ...
    //물론 특화의 문제이므로, if든 switch든 문법만 충족하면 자유롭게 써도 됩니다.
}