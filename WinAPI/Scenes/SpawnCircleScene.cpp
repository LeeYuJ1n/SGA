...
void SpawnCircleScene::Update()
{
    ...
    for (Bullet1* bullet : bullets)
    {
       ...
       // 테두리 충돌
        if (bullet->Pos().y - bullet->GetRadius() <= 0) // 총알이 위쪽 테두리 침범
        {
            ReflectBullet(bullet, true); // 수평 충돌 호출
        }

        if (bullet->Pos().x - bullet->GetRadius() <= 0 || // 총알이 왼쪽 침범
            bullet->Pos().x + bullet->GetRadius() >= WIN_WIDTH) // 이건 오른쪽 침범
        {
            ReflectBullet(bullet, false); // 수직 충돌 호출
        }

        if (bullet->Pos().y - bullet->GetRadius() >= WIN_HEIGHT) // 총알이 아래쪽으로 빠진 것
        {
            bullet->Stop();
            bullet->isActive = false;

            // 총알이 벡터 관리가 있었다며 여기서 추가 처리를 해주기 (생성에도 반영)
        }
    }
}
