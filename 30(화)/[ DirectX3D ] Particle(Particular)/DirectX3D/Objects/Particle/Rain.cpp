...
void Rain::Render()
{
	buffer->SetVS(10); // 지오메트리 셰이더 + 그림 출력 조건문 사용 (10 : 다른 색 반환 없음)
...
void Rain::GUIRender()
{
	...
	ImGui::Text("Rain Particle Option");
...
void Rain::Create()
{
	...
	FOR(particleCount)
	{
		...
		// size : 날씨 버퍼의 크기 = 날씨가 영향을 미치는 지역의 파티클 지역의 규모 = 파티클 전체 크기
