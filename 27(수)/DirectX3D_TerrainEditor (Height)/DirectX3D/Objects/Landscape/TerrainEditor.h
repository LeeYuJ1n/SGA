...
private:
    ...
    const float MIN_HEIGHT = -20.0f; // 최소 높이 같이 정해놓기 ( Value : -20.0 )
    ...
    private:
        struct Data
        {
            int type     = 0;           // 이 브러시의 타입 ( Value : 0 )
            Float3 pickingPos;          // 찍고 있는 좌표
            float range  = 5.0f;        // 기본 범위 ( Value : 5.0 )
            Float3 color = { 0, 0, 1 }; // 파란색. 0, 1, 0이면 초록색 ( Value : 0, 0, 1 )
        };
        Data data;
...
public:
    ...
    bool IsComputePick(Vector3& pos);    // 컴퓨트 픽과 역시 거의 같고, 
                                         ...
                                         // 반환형은 boolean을 주는 함수
    float GetHeightByPos(Vector3 myPos); // 매개변수가 주어지면 해당 데이터에 맞는 지형의 높이를주는 변수
