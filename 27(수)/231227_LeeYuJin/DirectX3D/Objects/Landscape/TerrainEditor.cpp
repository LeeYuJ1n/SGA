...
void TerrainEditor::Update()
{
    ...
    {
        // 지혀엥 마우스 커서가 올라가 있는 상황 ( 컴퓨트 픽이 true 반환 )
...
// 원형 컴퓨트 픽 ( 마우스 위치에서 지형 좌표 얻기 ) 위 함수의 bool 반환형 (+pos 저장)
...
void TerrainEditor::SaveHeightMap()
{
    ...
    if (DIALOG->Display("SaveHeight"))
    {
        if (DIALOG->IsOk())
        {
            ...
            // 이미지를 생성해서 해당 점 하나한에 정점 정보를 픽셀의 정보로 계산해서 넣기
...
// 높이 대신 알바 정보를 8바이트 이미지로 만들어서 저장 ( 그리고 아래 함수에서 읽기 )
