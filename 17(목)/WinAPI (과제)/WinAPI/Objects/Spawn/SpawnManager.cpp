...
SpawnManager::SpawnManager()
{
    ...
    //       또한 자료 구조 안에도 모든 요소들이(원소가) 똑같은 속성을 똑같이 가지고 있을 때
    //       그럴 때 써볼 수 있는 문법
...
//    -> "할당 후 관리" (control after allocation)을 사용
...
void SpawnManager::DestoryObject(Bullet1* bullet)
{
    for (SpawnObject* spawnObject : spawnObjects)
    {
        if (spawnObject->IsCollision(bullet))
        {
            spawnObject->Destroy();
            score++;
            return;
