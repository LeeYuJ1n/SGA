...
void SpawnBrute::Update()
{
    ...
    if (KEY_DOWN(VK_LBUTTON)) //마우스 왼쪽 버튼을 누르면
                              ...
                              //   = 그래서 마우스의 "물리적인 위치"는 생각을 덜 해도 될까요? - 됩니다!
    {
        //사각형을 스폰해봅시다.
        SpawnManager::Get()->spawnObjects.push_back(new SpawnObject()); //새 객체 생성/추가
        SpawnManager::Get()->spawnObjects[SpawnManager::Get()->spawnObjects.size() - 1]->Spawn(); //씬 내 자기 생성
    }

    // * 마우스 주 버튼을 "왼쪽 버튼"이라고 명명하는 것에는 두 가지 의미가 있습니다.
    //   1. 입력 하드웨어가 다르다고 가정할 때 (터치 스크린) 역시 똑같은 신호만
    //      줄 수 있으면 마우스 좌클릭과 똑같은 연산을 수행시킬 수 있습니다.
    ...
    for (int i = 0; i < SpawnManager::Get()->spawnObjects.size(); ++i)
    {
        if (SpawnManager::Get()->spawnObjects[i]->IsCollision(rect)) //부딪치면
        {
            ...
            SpawnManager::Get()->spawnObjects.erase(SpawnManager::Get()->spawnObjects.begin() + i); //벡터에서 대상 삭제
...
void SpawnBrute::Render(HDC hdc)
{
    ...
    TextOut(hdc, CENTER_X, 100, std::to_wstring(SpawnManager::Get()->spawnObjects.size()).c_str(), 2);
