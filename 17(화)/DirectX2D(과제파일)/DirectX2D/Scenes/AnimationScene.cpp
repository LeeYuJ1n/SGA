...
AnimationScene::AnimationScene()
{
    ...
    // Font::Get()->AddStyle("Gothic", L"고딕체", 50.0f);
    // 
    // Font::Get()->SetColor("Black");
    // Font::Get()->SetStyle("Gothic");

    // 2번째 스타일
    // Font::Get()->AddColor("Pink", 255, 0, 255);
    // Font::Get()->AddStyle("Malrang", L"한컴 말랑말랑", 50.0f);
    // 
    // Font::Get()->SetColor("Pink");
    // Font::Get()->SetStyle("Malrang");

    // 3번째 스타일
    Font::Get()->AddColor("Purple", 204, 0, 255);
    Font::Get()->AddStyle("Human Delinquency", L"휴먼편지체", 50.0f);
    
    Font::Get()->SetColor("Purple");
    Font::Get()->SetStyle("Human Delinquency");
}
...
void AnimationScene::Render()
{
    ...
    // 글꼴 출력
    Font::Get()->RenderText(L"도화가", { 100, WIN_HEIGHT - 100 });
