...
{
    rtvTexture->Release(); // 텍스처 클래스에 없고, 윈도우 식으로 만들었으니 윈도우 식으로 지우기
    rtv->Release();
    srv->Release();
}
...
void RenderTarget::SetMulti(RenderTarget** target, UINT count, DepthStencil* depthStencil, Float4 clearColor)
{
    ...
    {
        rtvs.push_back(target[i]->GetRTV()); //매개변수로 받은 렌더 타겟"들"의 i번째 값을 넣기
    ...
    Environment::Get()->Set();
    // 여러 개의 영역(위치와 크기)은 여기서는 직접 다루진 않는다
...
void RenderTarget::CreateRTVTexture()
{
    ...
    desc.MipLevels = 1;
    desc.ArraySize = 1;
    desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // 법선(자체 3D화) 없는 기본 RGB 이미지 형식
    desc.SampleDesc.Count = 1;
    desc.Usage = D3D11_USAGE_DEFAULT; // 기본 유형으로 사용
    desc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;

    DEVICE->CreateTexture2D(&desc, nullptr, &rtvTexture);
                         // 원본은 desc 구조체, 결과물은 이 클래스의 텍스처로
}
