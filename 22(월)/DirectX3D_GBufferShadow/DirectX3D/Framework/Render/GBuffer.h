...
class GBuffer
{
    // G 버퍼 : 지오메트리(모양) 버퍼. 전체 월드의 형태(공간과 씬의 형태)를
    //          빠르게 연산할 수 있도록 데이터를 모으고 분류하는 버퍼 클래스

    // -> 전체 월드의 형태를 빠르게 연산해서 어쩔 건가? -> 투사할 거다
    // -> 렌더 타겟을 더욱 컴퓨터 입장에서 빠르게, 사람 입장에서 보기 쉽게 분류하는 것
...
private:
    ...
    // 구조는 위 기본형과 같다 : 여러 개의 렌더 타겟을 설정해 가면서 서로 다른 화면을 띄우는 것이 목적
