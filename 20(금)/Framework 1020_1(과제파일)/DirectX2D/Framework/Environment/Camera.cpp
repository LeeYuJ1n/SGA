...
// 카메라 원리 :
// 1. 독립된 공간을 가진 트랜스폼을 만들어서, 그 위치가 뷰포트의 대상이 되도록
// 2. 기본은 winAPI 때와 비슷 : 차이는 카메라가(카메라를 통해) 출력하는 것이 아니라
//                             출력은 똑같은 뷰포트가 하는 것 뿐 (단지 참조 위치가 달라진다)

//                             -> 따라서, 만든 카메라를 바로 매니저에 띄워야 하거나 자동으로 써지는 것이 아니라
...
void Camera::Update()
{
    // 기준점이 있으면 Follow-Cam으로 동작, 아니면 Free-Cam으로 동작
...
void Camera::SetView()
{
    ...
    view = XMMatrixInverse(nullptr, world); // XMMatrixInverse : 위치나 방향을 뒤짚에서 설정 가능
...
void Camera::FollowMode()
{
    ...
    Pos() = target->Pos() - Vector2(CENTER_X, CENTER_Y);
}
