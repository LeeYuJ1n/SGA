...
RenderTarget::RenderTarget(UINT widht, UINT height)
...
void RenderTarget::CreateRTVTexture()
{
	...
	// 렌더 타켓을 위해서는 자료를 받은 후, 실질 출력 정보를 설정한다

	D3D11_TEXTURE2D_DESC desc = {};

	desc.Width            = width;
	desc.Height           = height;
	desc.MipLevels        = 1; // 밉 맵의 수준, 1 = "기본 수준 = 풀 사이즈(기본 크기)"를 뜻 한다
	desc.ArraySize        = 1;
	...
	// 만든 정보를 바탕으로 2D 텍스처 재가공 후 rtvTexture에 반영
