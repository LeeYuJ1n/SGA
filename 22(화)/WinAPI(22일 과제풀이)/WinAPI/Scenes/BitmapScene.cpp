...
void BitmapScene::Update()
{
	...
	{
		actIndex1 = actIndex1 - lastIndex1;
	}

	if (actIndex2 > lastIndex2)
	{
		actIndex2 = actIndex2 - lastIndex2;
	}
	...
	if (KEY_DOWN(VK_LBUTTON))
	{
		...
		// → 순번이 그림의 최대 순번을 넘어가면 안 될것! (위쪽에 조건문)

		if (actIndex1 > lastIndex1)
		{
			actIndex1 = actIndex1 - lastIndex1; // 감산 보정
			// actIndex1 = 0; // 직접 보정
		}

		if (actIndex2 > lastIndex2)
		{
			actIndex2 = actIndex2 - lastIndex2; // 감산 보정
			// actIndex2 = 0;
		}
	}
}
...
void BitmapScene::Render(HDC hdc)
{
	...
	GdiTransparentBlt(hdc,    // 출력할 화면(모니터)
		...
		128 * actIndex1, 0,   // 어디서 가져오려는가 (128 픽셀 기준으로 표시 주체가 어디까지 왔는가)
	...
	GdiTransparentBlt(hdc,    // 출력할 화면(모니터)
		...
		memDC,                // 어디에서 화면을 가져올 것인가
		128 * actIndex2, 0,   // 어디서 가져오려는가 (128 픽셀 기준으로 표시 주체가 어디까지 왔는가)
		128, 128,             // 원본의 그림을 어느 정도 크기로 가져올 것인가 (해당 안되면 자르기)
		RGB(255, 255, 255));  // 그림을 가져올 떄, ← 여기 지정된 색깔은 투명하게 만들라는 뜻
