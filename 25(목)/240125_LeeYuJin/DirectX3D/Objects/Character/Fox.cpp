...
{
	ReadClip("Idle");
	ReadClip("Run", 0, "FoxTransform"); // 모델 파일에 있던 뼈대를 직접 수동으로 설정

	PlayClip(curState); // 현재 상태의 따른 동작을 1회 호출
}
...
void Fox::Control()
{
	if (KEY_DOWN(VK_LBUTTON))
	{
		...
		else // 장애물이 없으면
		{
			...
			path.push_back(destPos); // 최종 목적지 하나만 남기기 = 그 쪽으로 Move() 함수를 수행
		}
	}
}
...
void Fox::Move()
{
	...
	Vector3 dest = path.back(); // 최종 목적지로부터 시작된 "길"의 마지막
	...
	// 목적지로 가기 위한 방향으로 이동
	velocity = direction.GetNormalized(); // 정규화
	Pos() += velocity * moveSpeed * DELTA;
}

void Fox::Rotate()
{
	if (curState == IDLE) return; // 이동 중이 아닐 떄는 회전 안함

	Vector3 forward = Forward();                // 모델의 앞 따오기
	Vector3 cross   = Cross(forward, velocity); // 내 앞과 가야 할 방향에서 나온 법선

	if (cross.y <= 0) // 위 계산의 의한 법선이 밑이면 = 내가 목적지보다 오른쪽을 보는 중이다
...
void Fox::SetPath()
{
	int startIndex = aStar->FindCloseNode(GlobalPos()); // 여우가 있는 노드가 시작 위치
	...
	while (path.size() > 2) // 이후 길을 찾아가면서 남은 경로 수가 2 이하가 될 때까지 아래 반복
	{
		...
		tempPath.erase(tempPath.begin()); // 최종 목적지는 일단 생각 안함 (지금은 중간 경로가 있어서)
		tempPath.pop_back(); // 이제 경로에서 가는 중이니까 시작 위치도 생각 안함 (이제는 "지나간 곳"이어서)
