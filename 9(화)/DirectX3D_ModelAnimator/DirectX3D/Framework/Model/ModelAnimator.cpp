...
void ModelAnimator::Render()
{
	...
	frameBuffer->SetVS(3); // 움직임의 정점 셰이터를 설정
...
void ModelAnimator::ReadClip(string clipName, UINT clipNum)
{
	...
	UINT boneCount = reader->UInt(); // 뼈개 개수도 읽어오기
	...
	FOR(boneCount) // 뼈 개수 동안
	{
		...
		if (size > 0) // 크기가 있다면 = 프레임이 있다면
		{
			...
			reader->Byte(&ptr, sizeof(KeyTransform) * size); // 안에 있는 트랜스폼 정보 만큼 읽기
		}
...
Matrix ModelAnimator::GetTransformByNode(int nodeIndex)
{
	...
	// 이 중괄호가 끝날 때마다 임시로 만들었던 변수드은 도로 사라진다
	{
		// 현재 움직임 연산
		...
		// 현재 계산의 결과를 밖에서 만든 curAnim에 적용
		curAnim = Lerp(cur, next, curFrame.time);
		// Lerp = Line Interprlation, 선형 보간
		// 보간 : 사이를 채워넣기
	}

	// 여기까지 오면 curAnim 준비 끝, 위쪽 {} 안에 있던 curFrame, cur, next 지워짐
...
void ModelAnimator::UpdateFrame()
{
	...
	// 중괄호 안에서 계산 수행
	{
		...
		clip->playTime += frameData.cur.scale * DELTA; // 얼마나 수행 되어야 하는가(규모)를 진행 시간에 누적합산

		frameData.cur.time += clip->tickPerSecond * frameData.cur.scale * DELTA; // 애니메이션 자체의 분량이 얼마나 진행되어야 하는가를 (위와 같음)
...

	{
		...
		// 여기까지 와다면 다음 프레임과 클립을 재생해야 할 텐데, 아직 전환이 다 되지 않은 것
