...
{
	// 파일의 대한 정보(파일 자체든, 파일에 의한 메모리 내 정보든)
	// -> 파일을 다룰 때는 빠뜨릴 수 없는 부분

	file = CreateFileA(filePath.c_str(), //파일의 경로명
		GENERIC_READ,                    // 읽기가 가능할 때 읽기
		FILE_SHARE_READ,                 // 파일과 별개로 저장소에도 읽기 권한이 있어야 일긱
		0,
...
string BinaryReader::String()
{
	...
	// 그렇기 때문에 이런 식으로 문자 배열에 거꾸로 값을 넣었을 때는, 배열의 끝을 "\0(문자 끝)"이라고 작성해서 나타내야 한다
