...
BinaryWriter::BinaryWriter(wstring filePath)
{
	...
	// 데이터가 빈 경우에도 미리 초기화된 파일을 작성
	file = CreateFile(filePath.c_str(), //파일의 경로명
		GENERIC_WRITE,                  // 쓰기 권한 액세스
		0,                              // 공유 모드 (보통은 그냥 0)
...
{
	// delete file;
	CloseHandle(file); // "delete file;" 코드와 역할은 같은데, 조금 더 안전
}
...
void BinaryWriter::String(string data)
{
	...
	UInt(data.size()); // 여기 들억간 data = string = 문자의 벡터
	...
	// 그런데 작성할 데이터는 string이므로 1차 기준에 문자 길이만큼 실제 데이터의 기준 크기를 도출
}
...
{
	// 무엇이든 대응해야 하므로 최대한 단순하게
	// -> 이 함수를 부를 때는 매개변수 넣는 쪽이 ( = 호출 측이) 형변환부터 필요한 연산을 다 해두어야 한다
