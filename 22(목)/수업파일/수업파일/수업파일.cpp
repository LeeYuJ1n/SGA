...
int main()
{
    ...
    // 다차원 배열에서는 오른쪽에 있는 []가 먼저 순번이 돌고, 그 다음에 더 왼쪽에 있는 []가가 순번이 돈다
    // 더 오른쪽에 있는 []가  더 작은 원소(부분 배열)를 뜻하고, 그래프로 치면 x축같은 역할을 한다
    
    // "다차원 배열"이라고 불리는 만큼...배열의 중첩이 2중으로만 국한되지 않는다
    // 배열의 차원은 중첩 가능한 만큼(메모리가 버티는 만큼)할 수 있다(현실적으로 그렇게 하는 경우는 많지 않다)
    int d3[1][2][3]; // 삼중배열
    int d4[1][2][3][4]; // 사중배열
    ...
    // → 반복문도 반복문 안에 또 만듬으로써 다차원 배열을 다룰 수 있습니다
    ...
    for (int i = 0; i < 10; ++i)
    {
        // 이 반복문은 0부터 10까지 돌아간다

        for (int j = 0; j < 10; ++j)
        {
            // i가 순번인 위쪽 for문이 한 번 돌아갈 때,
            // j가 순번인 이 반복문이 0부터 9까지 돌아간다
            // i가 0일 때, j가 0부터 9까지 한 번, 다시 i가 1이 되고 나서 j는 또 0부터 9까지 쭉, 반복된다
            // → 이 반복문은 10*10, 100번 돌아간다

            dec[i][j] = (i * 10) + (j + 1);
            // i - j번째 숫자에, i를 십의자리, j를 일의 자리로 설정한 숫자를 할당

            std::cout << dec[i][j] << std::endl;
        }

        std::cout << "i가 순번인 반복문이" << i + 1 << "번 돌았습니다" << std::endl;
        // i 는 0부터 9까지 i + 1하면 1부터 10까지 셀 수 있다
    }
    ...
    // 이 코드에서 어떤 부분을 고치면... 사각형을 삼각형으로 바꿀 수도 있습니다
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        {
            std::cout << " * ";
            // j가 돌아갈 때 한 가지 바꾸는 숫자 : i
            // 처음 이 반복문이 돌아갈 때 i = 0
            // 마지막으로 돌아갈 때 i = 9

            // 두 번째 for문의 조건이 j <= i가 된다면
            // 처음 돌아갈 때는 j가 0일 때 한 번만 돌고
            // 마지막 돌 때는 0부터 9까지 10번 다 돌게된다
        }
    ...
    for (int i = 0; i < 1000; ++i)
        // 이 반복문에서 i는 반복문 속에서 쓰는 변수가 아니라, 오직 for문이 돌아가라 횟수만을 뜻한다
        // 1000번 돌아가는 반복문
    {
        // 랜덤 숫자 2개 만들기
        int x1 = rand() % 9;
